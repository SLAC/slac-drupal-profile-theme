...
class YurtsHelpers {
  ...
  addYurtsButtonEvent() {
    if (this.yurtsDiv) {
      ...
      if (!this.searchButtonEventListenerAdded) {
                ...
                mutation.addedNodes.forEach(node => {
                  ...
                  if (searchButton) {
                    ...
                    searchButton.addEventListener('click', () => {
                      yurtsTextModifierObserver.observe(document, {
                        childList: true,
                        subtree: true,
                      });
                    });
        ...
        searchButtonObserver.observe(this.yurtsDiv, {
          childList: true,
          subtree: true,
        });
      }
  ...
  yurtsTextModifier() {
    return new MutationObserver((mutations, observer) => {
      mutations.forEach(mutation => {
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach(node => {
            const firstLineXpath =
              "//span[text()='The Gateway to Your Knowledge']";
            const firstLineTextElement = document.evaluate(
              firstLineXpath,
              node,
              null,
            ...
            if (firstLineTextElement) {
              firstLineTextElement.textContent =
                'SLAC IT has heard your feedback—and we are relaunching SLAC Search, now powered by GenAI/ML';
            }
            const secondLineXpath =
              "//span[text()='Ready to explore? Your search begins here.']";
            const secondLineTextElement = document.evaluate(
              secondLineXpath,
              node,
              null,
              XPathResult.FIRST_ORDERED_NODE_TYPE,
            ...
            if (secondLineTextElement) {
              secondLineTextElement.textContent =
  ...
  yurtsUnavailable() {
    if (this.yurtsDiv) {
      ...
      const unavailableMessage = document.createTextNode(
        'SLAC web search is temporarily unavailable.'
      );
